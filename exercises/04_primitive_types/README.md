# Types primitifs

Rust possède quelques types de base qui sont directement implémentés dans le compilateur.
compilateur. Dans cette section, nous allons passer en revue les plus importants d'entre eux.

## Plus d'informations

- [Types de données](https://doc.rust-lang.org/stable/book/ch03-02-data-types.html)
- [Le type de tranche](https://doc.rust-lang.org/stable/book/ch04-03-slices.html)
